...
function submit() {
  form.post(route('enquiry.store', props.business.slug), {
...
<template>
  ...
  <div class="min-h-screen bg-gray-100 px-4 py-10 dark:bg-slate-950">
    <div class="mx-auto max-w-2xl rounded-xl border bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ business.name }}</h1>
      <p class="mb-6 text-sm text-gray-600 dark:text-gray-400">Send us your enquiry and we will get back to you.</p>
      <p v-if="flash.success" class="mb-4 rounded bg-green-100 p-3 text-sm text-green-700">{{ flash.success }}</p>
